"""
Tests for gui.dashboard — TransferDashboard and StatsLabel.
"""
...
from gui.dashboard import TransferDashboard, StatsLabel
...
# ═══════════════════════════════════════════════════════════════════════════
# TransferDashboard — ETE format
# ═══════════════════════════════════════════════════════════════════════════

class TestETEFormat:

    def test_format_ete_zero(self):
        assert TransferDashboard._format_ete(0) == "\u2014"

    def test_format_ete_negative(self):
        assert TransferDashboard._format_ete(-10) == "\u2014"

    def test_format_ete_seconds(self):
        assert TransferDashboard._format_ete(45) == "0:45"

    def test_format_ete_minutes(self):
        assert TransferDashboard._format_ete(125) == "2:05"

    def test_format_ete_exactly_one_hour(self):
        assert TransferDashboard._format_ete(3600) == "1:00:00"

    def test_format_ete_hours(self):
        # 1h 23m 45s = 5025s
        assert TransferDashboard._format_ete(5025) == "1:23:45"

    def test_format_ete_large(self):
        # 10h 0m 0s
        assert TransferDashboard._format_ete(36000) == "10:00:00"


# ═══════════════════════════════════════════════════════════════════════════
# TransferDashboard — status helpers
# ═══════════════════════════════════════════════════════════════════════════

class TestStatusHelpers:

    def test_status_text_queued(self):
        text = TransferDashboard._status_text("queued")
        assert "Queued" in text

    def test_status_text_transferring(self):
        text = TransferDashboard._status_text("transferring")
        assert "Transferring" in text

    def test_status_text_done(self):
        text = TransferDashboard._status_text("done")
        assert "Done" in text

    def test_status_text_error(self):
        text = TransferDashboard._status_text("error")
        assert "Error" in text

    def test_status_text_skipped(self):
        text = TransferDashboard._status_text("skipped")
        assert "Skipped" in text

    def test_status_text_unknown(self):
        text = TransferDashboard._status_text("weirdo")
        assert text == "weirdo"

    def test_status_color_done_is_green(self):
        color = TransferDashboard._status_color("done")
        assert color.name() == "#2ecc71"

    def test_status_color_error_is_red(self):
        color = TransferDashboard._status_color("error")
        assert color.name() == "#e74c3c"

    def test_status_color_transferring_is_orange(self):
        color = TransferDashboard._status_color("transferring")
        assert color.name() == "#f39c12"

    def test_status_color_unknown(self):
        color = TransferDashboard._status_color("unknown")
        assert color.name() == "#d4d4d4"


# ═══════════════════════════════════════════════════════════════════════════
# TransferDashboard — widget creation and UI state
# ═══════════════════════════════════════════════════════════════════════════
...
class TestDashboardUI:
    ...
    def _create(self, default_config, qapp):
        self.dashboard = TransferDashboard(config=default_config)
    ...
    def test_spinboxes_default_values(self):
        assert self.dashboard.hours_spin.value() == 3  # default_hours
    ...
    def test_filter_checkbox_default(self):
        assert not self.dashboard.filter_enable_check.isChecked()
...
# ═══════════════════════════════════════════════════════════════════════════
# TransferDashboard — queue display
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# TransferDashboard — stats display
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# TransferDashboard — cycle events
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# TransferDashboard — filter group UI
# ═══════════════════════════════════════════════════════════════════════════
...
class TestDashboardFilterUI:
    ...
    def _create(self, populated_config, qapp):
        self.dashboard = TransferDashboard(config=populated_config)
    ...
    def test_settings_changed_marks_dirty(self):
        ...
        # restart_banner.setVisible(True) is called; in headless mode
        # isVisible() may still return False because the parent is hidden.
        # So we verify the _settings_dirty flag instead.
