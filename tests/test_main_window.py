"""
Tests for gui.main_window — MainWindow menu, service lifecycle, signal wiring.
"""
...
class TestMainWindowService:
    ...
    def test_start_without_remotes_warns(self, mock_warning):
    ...
    def test_start_creates_engine(self, MockEngine, mock_scp):
        ...
        MockEngine.assert_called_once_with(self.config)
    ...
    def test_stop_without_engine(self):
        # Should not crash
        self.win._on_stop_service()

    def test_stop_with_running_engine(self):
        mock_engine = MagicMock()
        mock_engine.is_running = True
        self.win.engine = mock_engine
        self.win._on_stop_service()
        mock_engine.stop.assert_called_once()

    def test_on_service_stopped_updates_dashboard(self):
        self.win._on_service_stopped()
        assert self.win.dashboard.btn_start.isEnabled()
...
class TestMainWindowUnknownInstitution:
    ...
    def test_assignment_saved_on_accept(self, MockPopup):
        ...
        mock_popup = MagicMock()
        # The source compares popup.exec() == UnknownInstitutionPopup.Accepted
        # When we patch the class, MockPopup.Accepted is a MagicMock.
        # We need exec() to return the same object that MockPopup.Accepted is.
